...
class FusionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        Inter_save_folder = './Inter_embeddings_{}/'.format(self.key)
        filename_str = filename[last_slash_index + 1:]
        filename_str = filename_str[:-4]
        Inter_embeddings = np.load(os.path.join(Inter_save_folder + filename_str + '_inter_embeddings.npy'))

        Intra_data, frame_inds = get_spatial_and_temporal_samples(filename, self.sample_types, self.samplers,
                                                                  self.phase == "train",
                                                                  self.augment and (self.phase == "train"),
                                                                  )     # Get F^{intra}

        guardian = (2 * np.random.randint(0, 2, size=(
                    Intra_data['fragments'].shape[0] * (Intra_data['fragments'].shape[2]) * (
            Intra_data['fragments'].shape[3]))) - 1)  # Initialized guardian map from {-1,1}
        guardian = torch.from_numpy(
            guardian.reshape(Intra_data['fragments'].shape[0], 1, (Intra_data['fragments'].shape[2]),
                             (Intra_data['fragments'].shape[3])))

        for k, v in Intra_data.items():
            Intra_data[k] = torch.clamp((Intra_data[k] + guardian), 0, 255)
            Intra_data[k] = ((v.permute(1, 2, 3, 0) - self.mean) / self.std).permute(3, 0, 1, 2)

        # #
        Intra_data["num_clips"] = {}
        for stype, sopt in self.sample_types.items():
            Intra_data["num_clips"][stype] = sopt["num_clips"]
        Intra_data["frame_inds"] = frame_inds
        Intra_data["gt_label"] = label
        Intra_data["name"] = osp.basename(video_info["filename"])

        return Intra_data, Inter_embeddings
...
class FusionDatasetK400(torch.utils.data.Dataset):
    def __init__(self, opt):
        ...
        if "t_frag" not in opt:
            ...
            self.sampler = FragmentSampleFrames(opt["clip_len"] // opt["t_frag"], opt["t_frag"], opt["frame_interval"],
                                                opt["num_clips"])
