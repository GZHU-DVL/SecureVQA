...
import securevqa.models_finetune as models
import securevqa.datasets_finetune as datasets
import argparse
...
def main():
    ...
    parser.add_argument(
        "-o", "--opt", type=str, default="./options/Finetune/konvid.yml", help="the option file"
    )
    ...
    dataset_key = "val-kv1k"
    for split in range(num_splits):

        model = getattr(models, opt["model"]["type"])(**opt["model"]["args"]).to(device)

        log("split{}:".format(split))

        split_duo = train_test_split(opt["data"]["train"]["args"]["data_prefix"],
                                     ann_path,
                                     seed=opt["split_seed"] * (split + 1))
        opt["data"]["train"]["args"]["anno_file"], opt["data"][dataset_key]["args"]["anno_file"] = split_duo

        val_datasets = {}
        for key in opt["data"]:
            if key.startswith("val"):
                val_datasets[key] = getattr(datasets,
                                            opt["data"][key]["type"])(opt["data"][key]["args"],dataset_key)
        ...
        for key in opt["data"]:
            if key.startswith("train"):
                train_dataset = getattr(datasets, opt["data"][key]["type"])(opt["data"][key]["args"],dataset_key)
...
log_path = 'logs/{}.txt'.format(('SecureVQA_finetune'))
def log(str_to_log):
    print(str_to_log)
