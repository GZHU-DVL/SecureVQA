...
def get_Lsrb_value(inds_list,model,video_data,label,Slowfast,q_hat_original_min,q_hat_original_max,median,config):  #Compute the Score-Reversed Boundary loss
    ...
    fake_score =  get_score(inds_list,video_data, Slowfast,model, 0, config)  #Compute the estimated quality score of adversarial video
...
def patch_based(inds_list,video_data,Slowfast, model, label, median, q_hat_original_min, q_hat_original_max, opt, config):  #Black-box attack on NR-VQA model
    Z_patches = max(int(video_data.shape[3] *(video_data.shape[1]  // config.patch_size) * (video_data.shape[2] // config.patch_size) / config.query),1) #Select Z patches to perturb in one frame.
    pn = video_data.shape[3] * (video_data.shape[1] // config.patch_size)  * (video_data.shape[2] // config.patch_size)
    patch_heigth = (video_data.shape[1] // config.patch_size)  #The number of patches in the vertical direction
    patch_width = (video_data.shape[2] // config.patch_size)   #The number of patches in the horizontal direction
    perm = torch.randperm(pn)  #Encode information of the positions of the selected patches
    last_prob, last_score, boundary = get_Lsrb_value(inds_list,model,video_data, label,Slowfast,q_hat_original_min,q_hat_original_max,median,config)  #Original quality score (estimated quality score)
    init_prob = last_prob
    ...
    for n in range(0,pn,Z_patches):  # One round contains N queries.
        ...
        for num_patch in range(Z_patches):  #Compute the position of the selected patch in the frame
            ...
            diff[:,(heigth_begin * config.patch_size):(heigth_begin * config.patch_size)+config.patch_size,(width_begin * config.patch_size)
            :(width_begin * config.patch_size)+config.patch_size,channel] = config.gamma * torch.from_numpy(m_n.reshape(1,config.patch_size,config.patch_size)).to('cuda')
        left_prob, left_score, boundary = get_Lsrb_value(inds_list,model,(video_data-diff).clamp(0,1), label,Slowfast,q_hat_original_min,q_hat_original_max,median,config)  #Evaluate the attack effect.
        if left_prob < last_prob:
            video_data = (video_data - diff).clamp(0,1) #Update the adversarial video.
            last_prob = left_prob
            last_score = left_score
        else:
            right_prob, right_score, boundary = get_Lsrb_value(inds_list,model,(video_data+diff).clamp(0,1), label,Slowfast,q_hat_original_min,q_hat_original_max,median,config) #  Evaluate the attack effect.
...
def get_score(inds_list,video_data,Slowfast, model, flag, config):
    '''
...
def do_attack(inf_loader, model, device, opt, max_score,config):
    ...
    for i, (filename, label) in enumerate(tqdm(inf_loader, desc="Training")):
        ...
        with torch.no_grad():
            sa = get_score(inds_list, video_data[0:len(video_data)] / 255, Slowfast, model, 1, config)  # Original quality score (estimated quality score)
    ...
    for video_index in range(len(q_mos)):
        ...
        if config.attack_trigger == 1:
            for index in range(0, len(frame_index)):
                ...
                adv_round, boundary = patch_based(inds_list, video_data_round, Slowfast, model, q_mos[video_index],
                                                                                                  median,
                                                                                                  q_hat_original_min,
                                                                                                  q_hat_original_max,
                                                                                                  opt, config
                ...
                perturbations[frame_index[index]:frame_index[index] + 1] = adv_round.cpu().detach() - video_data_round.cpu().detach()
            with torch.no_grad():
                sa_adv = get_score(inds_list, adversarial_video, Slowfast, model, 1, config)    #Compute the estimated quality score of adversarial video
...
log_path = 'logs/{}.txt'.format(('Black-SecureVQA'))
def log(str_to_log):
    print(str_to_log)
