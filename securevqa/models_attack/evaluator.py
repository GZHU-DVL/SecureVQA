...
class DiViDeAddEvaluator(nn.Module):
    ...
    def forward(self, vclips,Inter_embeddings, inference=True, return_pooled_feats=False, reduce_scores=True, pooled=False, **kwargs):
        if inference:
            # self.eval()
            # with torch.no_grad():
            ...
            for key in vclips:
                ...
                if hasattr(self, key.split("_")[0]+"_head"):
                    ...
                    scores += [getattr(self, "vqa_head")(feat,Inter_embeddings)]
